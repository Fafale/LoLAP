        ...
        for file in files:
            if file.startswith("AP"):
                with open(os.path.join(game_communication_path, file), 'r') as f:
                    ...
                    elif item_id % 565999999 == 0:
                        game_values["current_checks"] = game_values["current_checks"] + 1
                    else:
...
def read_cfg(game_values):
    for root, dirs, files in os.walk(game_communication_path):
        ...
        if "Required_Checks.cfg" in files:
            with open(os.path.join(game_communication_path, "Required_Checks.cfg"), 'r') as f:
                game_values["required_checks"] = int(f.readline())
        else:
            game_values["required_checks"] = 0
...
def display_values(window, game_values):
    ...
    value_table_rows.append(['Required Checks:'      , str(game_values["required_checks"])])
    value_table_rows.append(['Current Checks:'       , str(game_values["current_checks"])])
...
def check_lp_and_checks_for_victory(game_values):
    if game_values["current_lp"] >= game_values["required_lp"] and game_values["required_lp"] != 0:
        if game_values["current_checks"] >= game_values["required_checks"] and game_values["required_checks"] != 0:
...
while True:
    ...
    if event == 'Check for Match Button':
        in_match = True
    check_lp_and_checks_for_victory(game_values)
