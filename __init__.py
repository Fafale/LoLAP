...
class LOLWorld(World):
    ...
    def set_rules(self):
        self.choose_possible_champions()
        set_rules(self.multiworld, self.player, self.options, int(self.added_lp * (self.options.required_lp / 100)), self.possible_champions, int(self.added_checks * (self.options.total_check_amount / 100)))
    ...
    def fill_slot_data(self) -> dict:
        slot_data = {"Required CS":      int(self.options.required_creep_score)
                    ...
                    ,"Required Checks":  int(self.added_checks * (self.options.total_check_amount / 100))}
        return slot_data
